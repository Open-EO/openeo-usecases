...
def apply_hypercube(cube: DataCube, context: Dict) -> DataCube:
    ...
    from parcel.pixel.cropsar.training import models
    import xarray
    import numpy
    import itertools
    from print_xdatacubexarray import print_xarray_dataarray
    ...
    def process_window(window, acquisitiondate, inarr, windowsize=128, nodata=0):
...
#         inarr=inarr.ffill(dim='t').resample(t='1D').ffill().sel({'t': output_index}, method='ffill')
        ...
        # Run neural network
        predictions = ganmodel.generator.predict((s1_backscatter, s2_ndvi, probav_ndvi))
...
#     print(PVid+": "+str(float(PV.min()))+" "+str(float(PV.max())))
    ...
    # load the model
    ganmodel = models.separateencoderPix2Pix(tslength=37)
    ganmodel.generator.load_weights(prediction_model)
    ...
    for iwin in windowlist:
        ires = process_window(iwin, middate, inarr.sel({'x':range(iwin[0][0],iwin[0][1]),'y':range(iwin[1][0],iwin[1][1])}), 128, 0.)
        predictions.loc[{'x':range(iwin[0][0],iwin[0][1]),'y':range(iwin[1][0],iwin[1][1])}]=ires
    
    print_xarray_dataarray('PREDICTIONS',predictions)
