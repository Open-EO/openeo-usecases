...
from phenology_usecase import udf_savitzkygolaysmooth_phenology, udf_phenology_old
...
class TestPhenology(unittest.TestCase):
    ...
    def test_singleBand(self):
        ...
        outcube=udf_savitzkygolaysmooth_phenology.apply_datacube(self.inpcube, dict(do_smoothing=False,do_phenology=True))
    ...
    def test_multiBand(self):
        ...
        outcube=udf_savitzkygolaysmooth_phenology.apply_datacube(inpcube, dict(do_smoothing=False,do_phenology=True))
    ...
    def test_oldnewPhenologyIsSame(self):
        optcube=udf_savitzkygolaysmooth_phenology.apply_datacube(DataCube(self.inpcube.get_array().drop(['x','y'])), dict(do_smoothing=False,do_phenology=True))
